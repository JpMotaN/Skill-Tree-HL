...
Engine.nenStage = function () {
  ...
  return { pn, name: stageFromPN(pn) };
    ...
    function iconPath(node) {
      ...
      // se já está absoluto/relativo, usa direto; se for só o nome, prefixa a pasta
    ...
    function refresh() {
      ...
      for (const id of Engine.state.active) {
        const li = document.createElement('li'); li.textContent = nodesById[id].label; ul.appendChild(li);
      }
      ...
      for (const id of Engine.state.active) {
        ...
        if (n && Array.isArray(n.techniques) && n.techniques.length) for (const t of n.techniques) techniques.push(t);
      }
